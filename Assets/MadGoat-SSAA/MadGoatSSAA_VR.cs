...
namespace MadGoat_SSAA {
    ...
    public class MadGoatSSAA_VR : MadGoatSSAA {
        ...
        private void ClearStandardCB() {
            if ((new List<CommandBuffer>(currentCamera.GetCommandBuffers(CameraEvent.AfterEverything))).Find(x => x.name == "SSAA_VR_APPLY") != null) {
                //RenderTexture.ReleaseTemporary(buff);
                currentCamera.RemoveCommandBuffer(CameraEvent.AfterEverything, processBuffer);
            }
        } 
