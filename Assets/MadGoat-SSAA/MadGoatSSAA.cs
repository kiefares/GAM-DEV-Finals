...
#region MonoBehaviour Events Methods
        ...
        private void Update()
        {
            ...
            renderCamera.depth = currentCamera.depth;
            ...
            SSAA_Internal.OnMainFilterChanged(filterType);
            FpsData.Update();
            SendDbgInfo();
        }
        private void OnDisable()
        {
            ...
            if (!MadGoatSSAA_Utils.DetectSRP())
            {
                ...
                if ((new List<CommandBuffer>(currentCamera.GetCommandBuffers(CameraEvent.BeforeImageEffects))).Find(x => x.name == "SSAA_Grab_Alpha") != null)
                {
                    ...
                    GrabAlphaCB.Clear();
                    currentCamera.RemoveCommandBuffer(CameraEvent.AfterImageEffects, (new List<CommandBuffer>(currentCamera.GetCommandBuffers(CameraEvent.AfterImageEffects))).Find(x => x.name == "SSAA_Apply_Alpha"));
                    PasteAlphaCB.Clear();
                }
            }
        }
...
#region MadGoat SSAA Private API
        ...
        private void DoAlphaCommandBuffer()
        {
            ...
            if ((new List<CommandBuffer>(currentCamera.GetCommandBuffers(CameraEvent.BeforeImageEffects))).Find(x => x.name == "SSAA_Grab_Alpha") == null)
            {
                ...
                currentCamera.AddCommandBuffer(CameraEvent.AfterImageEffects, PasteAlphaCB);
        ...
        private void RenderPanorama()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                {
                    SSAA_Internal.MaterialBilinear.SetFloat("_ResizeWidth", internalRes);
                    SSAA_Internal.MaterialBilinear.SetFloat("_ResizeHeight", internalRes);

                    SSAA_Internal.MaterialBilinear.SetFloat("_Sharpness", panoramaSettings.sharpness);
                    Graphics.Blit(sphereTemp, buffer, SSAA_Internal.MaterialBilinear, 0);
                }
