...
[CustomEditor(typeof(PrismVolume), true)]
[CanEditMultipleObjects]
public class PrismVolumeEditor : Editor {

	//GUIContent volShapeContent = new GUIContent ("Volume Shape", "Shape of the PRISM Volume");
	//GUIContent volPresetContent = new GUIContent ("Volume Preset", "PRISM preset that the volume will use");
	/*
	public override void OnInspectorGUI() {
		serializedObject.Update ();
		
		PrismVolume volume = target as PrismVolume;

		EditorGUILayout.LabelField("Volume Settings", EditorStyles.boldLabel);
		EditorGUILayout.Space();

		EditorGUI.BeginChangeCheck ();
		PrismVolume.VolumeShape volShape = (PrismVolume.VolumeShape)EditorGUILayout.EnumPopup (volShapeContent, volume.volumeShape);
		if (EditorGUI.EndChangeCheck ()) {
			Undo.RecordObject (target, "Vol shape");
			volume.volumeShape = volShape;
		}

		EditorGUI.BeginChangeCheck ();
		PrismPreset preset = (PrismPreset)EditorGUILayout.ObjectField (volPresetContent, volume.volumePreset, typeof(PrismPreset), false);
		if (EditorGUI.EndChangeCheck ()) {
			Undo.RecordObject (target, "Change prism preset");
			volume.volumePreset = preset;
		}
		
		
		EditorGUILayout.LabelField("Target Camera Settings", EditorStyles.boldLabel);
		EditorGUILayout.Space();

		EditorGUILayout.LabelField("Timing Settings", EditorStyles.boldLabel);
		EditorGUILayout.Space();

		EditorGUILayout.LabelField("Tether Settings", EditorStyles.boldLabel);
		EditorGUILayout.Space();
		...
		ShowOnEnum("volumeShape","BOX","volumeSize"); //type1Var is only visible when type == Type1
		ShowOnEnum("volumeShape","SPHERE","volumeRange"); //type2Var is only visible when type == Type2
